...
app.listen(port, err => {
      ...
      const EXTRA_LONG_CACHE_PERIOD = 6 * 60 * 60 * 1000; // 6 hours

      const cacheNasa = async () => {
            await nasa();
            setTimeout(cacheNasa, EXTRA_LONG_CACHE_PERIOD);
      };

      const cacheSupercars = async () => {
            await supercars();
            setTimeout(cacheSupercars, EXTRA_LONG_CACHE_PERIOD);
      };
      ...
      const cacheDribbble = async () => {
            await dribbble();
            setTimeout(cacheDribbble, EXTRA_LONG_CACHE_PERIOD);
      };
